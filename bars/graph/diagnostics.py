...
def run_boundary_diagnostics(graph: BARSGraph, boundary: Optional[BoundaryIndex], cfg: Dict, logger: CSVLogger) -> None:
    ...
    p=np.asarray(psis,dtype=np.float32); logger.log({'phase':'boundary_diag','event':'completed','enabled':1,'num_pairs':len(p),'psi_mean':float(p.mean()),'psi_p10':float(np.quantile(p,0.10)),'psi_p50':float(np.quantile(p,0.50)),'psi_p90':float(np.quantile(p,0.90)),'supported_pair_rate':float(np.mean(supported))})
def run_path_diagnostics(dataset: OfflineDataset, embeddings: np.ndarray, graph: BARSGraph, boundary: Optional[BoundaryIndex], cfg: Dict, logger: CSVLogger) -> None:
    ...
    logger.log({'phase':'path_diag','event':'start','num_pairs':num_pairs,'variants':variants})
    ...
    for pair_id in range(num_pairs):
        ...
        if ((pair_id + 1) % progress_every) == 0 or pair_id == num_pairs - 1:
            logger.log({'phase':'path_diag','event':'progress','pairs_done':pair_id + 1,'num_pairs':num_pairs})
    logger.log({'phase':'path_diag','event':'completed','num_pairs':num_pairs,'variants':variants})
