...
def build_edges(dataset: OfflineDataset, embeddings: np.ndarray, node_indices: np.ndarray, reach_model: Optional[ReachabilityModel], cfg: Dict, device, logger: CSVLogger) -> BARSGraph:
    ...
    graph=BARSGraph(node_indices,node_emb,src,dst,cost,risk,p_exec,kind); logger.log({'phase':'edges','num_nodes':graph.num_nodes,'num_edges':graph.num_edges,'mean_out_degree':graph.num_edges/max(1,graph.num_nodes),'p_exec_mean':float(np.mean(p_exec)) if len(p_exec) else 0.0,'p_exec_p10':float(np.quantile(p_exec,0.10)) if len(p_exec) else 0.0,'risk_mean':float(np.mean(risk)) if len(risk) else 0.0,'cost_mean':float(np.mean(cost)) if len(cost) else 0.0}); return graph
