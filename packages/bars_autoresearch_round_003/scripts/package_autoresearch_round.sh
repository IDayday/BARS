...
cp scripts/round003_*.py "$PKG_ROOT/scripts/" 2>/dev/null || true
cp scripts/round003_*.sh "$PKG_ROOT/scripts/" 2>/dev/null || true
...
find commands -maxdepth 1 -type f -name "${RN}_*" -exec cp {} "$PKG_ROOT/commands/" \; 2>/dev/null || true
