...
## Phase 5N V0: Planner-Aware Low-Level GCBC

### Improvement Attempt

Phase 5N V0 implements the first BARS-native policy-training route that uses
the BARS planner target distribution directly. The training data mixes
final-goal hindsight, support-edge local goals, and planner-first-edge replay.
Planner replay is generated by running support-only graph planning over sampled
start/goal cluster queries, counting which support edges are used as immediate
subgoals, and upweighting/sampling those edges.

The important invariant is that planner replay never creates an unsupported
target. Every supervised example is still an observed trajectory segment
`(s_u, s_T, a_u)`.

### Result

AntMaze H10 B120 smoke:

- 200 training steps on `gcrlo` with `CUDA_VISIBLE_DEVICES=0`.
- `val_action_mse` decreases from `0.658260` at step 1 to `0.276327` at step
  200.
- `planner_first_edge_replay_val_mse` at step 200 is `0.271213`.
- `support_edge_local_val_mse` at step 200 is `0.277196`.
- `final_goal_hindsight_val_mse` at step 200 is `0.283521`.
...
This is a structural improvement over prior low-level training attempts because
the planner is now part of the supervised target distribution. It directly
...
- Stage36 adds the direct bridge from BARS graph evidence to official GAS:
  patch the official GAS keygraph edge weights or prune unsupported non-goal
