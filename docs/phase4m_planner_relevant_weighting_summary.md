...
`core_plus_bottleneck_budget192_H5` and AntMaze H10
...
| mean | 0.987 | 0.982 | 0.984 | 1.005 |

Scene assigns the highest mean weight to the 48 planner-used repair edges
(`1.217` mean loss weight). AntMaze assigns the highest repair-subgroup weight
to the 21 planner-used repair edges (`1.151` mean loss weight). In both cases
weights remain clipped below `2.1`.

## Analysis

This is a positive two-environment offline supervised result. Adding planner
relevance did not damage overall validation MSE; it improved ordinary
validation MSE, direct repair-edge MSE, and the planner-used repair-edge group
on both available repaired graphs.

The result should be treated as an offline supervised proxy. It does not prove
closed-loop option execution. The remaining replication gaps are Scene H10/H25,
