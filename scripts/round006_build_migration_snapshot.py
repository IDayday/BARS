...
def main() -> int:
    ...
    for row in rows:
        ...
        status_rel = copy_if_exists(status_json if status_json.exists() else None, snapshot_root / "status" / env / f"seed{seed}" / "status.json")
        status = ""
        ...
        completed.append(
            {
                ...
                "snapshot_status_json": "" if not status_rel else str(Path(status_rel).relative_to(args.snapshot_root)),
                "status": status,
            }
        )
