...
DEFAULT_ARTIFACT_ROOT = (
    REPO_ROOT
    / "artifacts"
    / "gas_ogbench_stage44_humanoid_visual_retrain_20260615_231837"
    / "humanoidmaze-large-navigate-v0"
    / "seed0"
)
DEFAULT_ENV_NAME = "humanoidmaze-large-navigate-v0"
...
DEFAULT_VARIANTS = [
    ...
    Variant(
        name="filtered_base",
        scored_rows_rel=(
            "runs_stage54_actor_visual_density/20260616_144721/"
            "sweep_v3/humanoid_ratio_0p5/scorer/contract_scored_rows.csv"
        ),
    ),
    Variant(
        name="filtered_actor_raw",
        scored_rows_rel=(
            "runs_stage57_humanoid_filtered_actor/20260616_165935/"
            "scorer/actor_raw/contract_scored_rows.csv"
        ),
    ),
    Variant(
        name="filtered_actor_conditioned",
        scored_rows_rel=(
            "runs_stage57_humanoid_filtered_actor/20260616_165935/"
            "scorer/actor_conditioned/contract_scored_rows.csv"
        ),
    ),
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Launch Stage58 humanoid local-graph smoke evaluations on one GPU. "
            "Patch generation stays offline-only; rollouts are used only for reporting."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output root. Defaults to runs_stage58_humanoid_filtered_eval_gpu3/<utc stamp>.",
    )
    parser.add_argument(
        "--variants",
        default="all",
        help="Comma-separated variants from {original,filtered_base,filtered_actor_raw,filtered_actor_conditioned}.",
    )
    parser.add_argument("--run-eval-project", default="stage58_humanoid_filtered_eval")
...
def selected_variants(value: str) -> list[Variant]:
    ...
    selected = [variant for variant in DEFAULT_VARIANTS if variant.name in wanted]
    missing = sorted(wanted - {variant.name for variant in selected})
    if missing:
        raise SystemExit(f"Unknown Stage58 variants: {','.join(missing)}")
    return selected
...
def prepare(args: argparse.Namespace) -> None:
    if "," in str(args.gpu):
        raise SystemExit("Stage58 eval is constrained to one GPU; pass one --gpu id.")
...
def policy_path(args: argparse.Namespace) -> Path:
    path = args.artifact_root / "policy" / "params_1000000.pkl"
    if not path.exists():
        raise SystemExit(f"Missing policy checkpoint: {path}")
    return path
...
def variant_keygraph(
    *,
    ...
    scored_rows = resolve_repo_path(Path(variant.scored_rows_rel))
    ...
    patch_cmd = [
        ...
        "--edge-scores-csv",
        str(edge_scores_dir / "caplite_edge_scores.csv"),
        "--out-keygraph-path",
...
def main() -> None:
    ...
    if args.out_root is None:
        ...
        args.out_root = REPO_ROOT / "runs_stage58_humanoid_filtered_eval_gpu3" / stamp
    else:
    ...
    latest = REPO_ROOT / "runs_stage58_humanoid_filtered_eval_gpu3" / "latest"
    ...
    env = build_env(args)
    status_path = args.out_root / "stage58_humanoid_filtered_eval_status.csv"
