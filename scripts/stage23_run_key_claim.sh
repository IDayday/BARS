...
CONFIG="${CONFIG:-configs/stage23_key_claim_reachability.json}"
GPUS="${GPUS:-0}"
WAIT="${WAIT:-1}"
...
STAGE23_ROOT="${STAGE23_ROOT:-runs_stage23_key_claim}"
LOG_ROOT="${LOG_ROOT:-runs_stage23_key_claim_logs}"
...
print("ENVS_CONF="+",".join(c["envs"]))
print("SEEDS_CONF="+",".join(map(str,c["seeds"])))
...
ARTIFACT_ROOT="$GAS_ARTIFACT_ROOT" ENVS="$ENVS" SEEDS="$SEEDS" GPUS="$GPUS" QUICK="$QUICK" PREFER_PRETRAINED="$PREFER_PRETRAINED" TRAIN_IF_MISSING="$TRAIN_IF_MISSING" LOG_ROOT="runs_stage23_prepare" bash scripts/stage22_prepare_gas_backbone.sh
ENVS="$ENVS" SEEDS="$SEEDS" GPUS="$GPUS" QUICK="$QUICK" GAS_ARTIFACT_ROOT="$GAS_ARTIFACT_ROOT" ARTIFACT_ROOT="$ARTIFACT_ROOT" GAS_REPO_PATH="$GAS_REPO_PATH" PREFER_PRETRAINED="$PREFER_PRETRAINED" TRAIN_IF_MISSING="$TRAIN_IF_MISSING" LOG_ROOT="runs_stage23_reachability" bash scripts/stage22_train_reachability.sh

jobs_tsv="$LOG_ROOT/jobs.tsv"
python - "$CONFIG" > "$jobs_tsv" <<'PY'
import json, os, sys
from pathlib import Path
c=json.load(open(sys.argv[1]))
envs = [x for x in os.environ.get("ENVS", ",".join(c["envs"])).split(",") if x]
seeds = [int(x) for x in os.environ.get("SEEDS", ",".join(map(str, c["seeds"]))).split(",") if x]
episodes = int(os.environ.get("EPISODES", c.get("episodes", 100)))
require_recommended = os.environ.get("REQUIRE_RECOMMENDED_BUDGET", "1") == "1"
...
for env in envs:
  for seed in seeds:
    ...
    for variant in c["variants"]:
      if budget is None and "shortest" not in variant:
        if require_recommended:
          continue
        budget=3.0
      variant_budget = budget if budget is not None else 0.0
      for fallback in c["fallback_modes"]:
...
while IFS=$'\t' read -r env seed variant budget fallback episodes; do
  ...
  out_dir="$STAGE23_ROOT/$env/seed$seed/$variant/budget$budget/fallback_$fallback"
  ...
  (
    ...
    python -m bars.gas_bars.evaluate \
      ...
      --gas-artifact-root "$GAS_ARTIFACT_ROOT" --artifact-root "$ARTIFACT_ROOT" --stage22-root "$STAGE23_ROOT" \
  ...
  if [[ "$active" -ge "$MAX_PARALLEL_EVAL" ]]; then
    wait -n || overall=1
    active=$((active - 1))
    python scripts/stage23_monitor_and_adjust.py --roots "$LOG_ROOT,$STAGE23_ROOT" --summary-md reports/stage23_live_summary.md || true
  fi
done < "$jobs_tsv"
...
python scripts/analyze_stage23.py --eval-root "$STAGE23_ROOT" --artifact-root "$ARTIFACT_ROOT" --out reports || true

if [[ "$WAIT" == "1" ]]; then
  python scripts/stage23_monitor_and_adjust.py --roots "$LOG_ROOT,$STAGE23_ROOT" --summary-md reports/stage23_live_summary.md --wait || true
fi
exit "$overall"
