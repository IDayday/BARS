...
CONFIG="${CONFIG:-configs/stage25_reachability_closing.json}"
GPUS="${GPUS:-0}"
WAIT="${WAIT:-1}"
...
STAGE25_ROOT="${STAGE25_ROOT:-runs_stage25_reachability_closing}"
LOG_ROOT="${LOG_ROOT:-runs_stage25_reachability_closing_logs}"
...
SKIP_REACHABILITY_TRAIN="${SKIP_REACHABILITY_TRAIN:-0}"
...
print("EPISODES_CONF="+str(c.get("episodes",300)))
print("MAX_PARALLEL_CONF="+str(c.get("max_parallel_eval",4)))
print("VARIANTS_CONF="+",".join(c.get("variants", ["gas_shortest","gas_reachability_budget_calibrated"])))
print("FALLBACK_CONF="+",".join(c.get("fallback_modes", ["none"])))
print("MIN_EPISODES_CONF="+str(c.get("min_episodes_for_gate", c.get("episodes",300))))
PY
)"
...
if [[ "$SKIP_PREPARE" != "1" ]]; then
  ...
  echo "[stage25_reachability] skip prepare (SKIP_PREPARE=1)"
fi
if [[ "$SKIP_REACHABILITY_TRAIN" != "1" ]]; then
  ...
  echo "[stage25_reachability] skip reachability scoring/training (SKIP_REACHABILITY_TRAIN=1)"
fi
...
for env in envs:
  for seed in seeds:
    ...
    for variant in variants:
      if budget is None and "reachability" in variant and require:
        continue
...
while IFS=$'\t' read -r env seed variant budget fallback episodes; do
  ...
  if [[ "$active" -ge "$MAX_PARALLEL_EVAL" ]]; then
    wait -n || overall=1
    active=$((active - 1))
    python scripts/stage25_analyze.py --reachability-roots "$STAGE25_ROOT" --reports-root "$REPORTS_ROOT" --min-episodes "$ANALYZE_MIN_EPISODES" || true
  fi
done < "$jobs_tsv"
...
python scripts/stage25_analyze.py --reachability-roots "$STAGE25_ROOT" --reports-root "$REPORTS_ROOT" --min-episodes "$ANALYZE_MIN_EPISODES" || true
if [[ "$WAIT" == "1" ]]; then
  echo "[stage25_reachability] reports written under $REPORTS_ROOT; root=$STAGE25_ROOT"
fi
exit "$overall"
