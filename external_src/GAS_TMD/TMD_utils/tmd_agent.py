...
from TMD_utils.tmd_encoders import GCEncoder, encoder_modules
from TMD_utils.tmd_flax_utils import ModuleDict, TrainState, nonpytree_field
from TMD_utils.tmd_networks import (
...
class TMDAgent(flax.struct.PyTreeNode):
    ...
    def get_psi(self, observations):
        """Return psi(s), averaging the official ensemble when present."""
        psi = self.network.select('psi')(observations)
        if len(psi.shape) == 3:
            psi = jnp.mean(psi, axis=0)
        elif len(psi.shape) == 2 and len(observations.shape) == 1:
            psi = jnp.mean(psi, axis=0)
        return psi

    @jax.jit
    def get_tmd_distance_from_embeddings(self, x, y):
        """Return directed TMD distance d_TMD(x, y)."""
        return self.distance(x, y)

    @jax.jit
    def get_tmd_distance(self, observations, goals):
        """Return directed TMD distance d_TMD(psi(observations), psi(goals))."""
        return self.get_tmd_distance_from_embeddings(self.get_psi(observations), self.get_psi(goals))
    ...
    def sample_actions(
        ...
        if seed is None or temperature == 0.0:
            actions = dist.mode()
        else:
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            way_steps=8,  # Temporal horizon used by GAS-TMD calibration/graph scripts.
